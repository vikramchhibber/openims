...
int P_release_call_onreply(struct sip_msg *msg,char *str1,char *str2)
{
	...
	if (is_p_dialog_dir(callid,dir)) {
		...
		if (t->method.len==6 && memcmp(t->method.s,"INVITE",6)==0)
		{
			...
			{
				ret = release_call_previous(d,RELEASE_CALL_WEIRD,488,reason);
			} else {
				ret = release_call_previous(d,RELEASE_CALL_EARLY,488,reason);
			}
			/* @luminate - check to see if dialog is valid */
            if (ret) {
