/**
 * $Id: dlg_state.c 920 2010-03-05 19:07:50Z vingarzan $
 *  
...
extern int mgcf_dialogs_expiration_time;	/**< expiration time for a dialog		*/
extern int mgcf_dialogs_enable_release;		/**< if to enable dialog release		*/
time_t d_time_now;							/**< current time for dialog updates 	*/

extern str mgcf_record_route_mo;			/**< Record-route for originating case 	*/
extern str mgcf_record_route_mo_uri;		/**< URI for Record-route originating	*/ 
extern str mgcf_record_route_mt;			/**< Record-route for terminating case 	*/
extern str mgcf_record_route_mt_uri;		/**< URI for Record-route terminating	*/

extern str mgcf_name_str;					/**< fixed SIP URI of this P-CSCF 		*/

extern struct tm_binds tmb;
extern int mgcf_min_se;
...
int m_dialogs_init(int hash_size)
{
	...
	m_dialogs_hash_size = hash_size;
	m_dialogs = shm_malloc(sizeof(m_dialog_hash_slot)*m_dialogs_hash_size);

	if (!m_dialogs) return 0;

	memset(m_dialogs,0,sizeof(m_dialog_hash_slot)*m_dialogs_hash_size);
	
	for(i=0;i<m_dialogs_hash_size;i++){
		m_dialogs[i].lock = lock_alloc();
		if (!m_dialogs[i].lock){
			LOG(L_ERR,"ERR:"M_NAME":d_hash_table_init(): Error creating lock\n");
			return 0;
		}
		m_dialogs[i].lock = lock_init(m_dialogs[i].lock);
	}
...
{
	lock_release(m_dialogs[(hash)].lock);
...
extern int* mgcf_dialog_count;
extern int mgcf_max_dialog_count;
extern gen_lock_t* mgcf_dialog_count_lock;

/**
 * Locks the dialog counter variable
 */
inline void m_dialog_count_lock()
{
 ...
 */
inline void m_dialog_count_unlock()
{
        lock_release(mgcf_dialog_count_lock);
}
 ...
 */
inline int m_dialog_count_increment ()
{
    if (mgcf_max_dialog_count<0) return 1;
    m_dialog_count_lock();	
	if (*mgcf_dialog_count<mgcf_max_dialog_count){
    	(*mgcf_dialog_count)++;
    	m_dialog_count_unlock();
    	return 1;
	} else {
    	m_dialog_count_unlock();
    	return 0;
	}
	LOG(L_DBG,"DBG:"M_NAME":m_dialog_count_increment(): P-CSCF Dialog counter value is %d\n", *mgcf_dialog_count);
}

/**
 * Decrement the dialog count
 */
inline void m_dialog_count_decrement()
{
    if (mgcf_max_dialog_count<0) return ;
    m_dialog_count_lock();
    (*mgcf_dialog_count)--;
    m_dialog_count_unlock();
	LOG(L_DBG,"DBG:"M_NAME":m_dialog_count_decrement(): P-CSCF Dialog counter value is %d\n", *mgcf_dialog_count);    
}


/**
 * Creates a new m_dialog structure.
 * Does not add the structure to the list.
 * @param call_id - call-id of the dialog
 * @returns the new m_dialog* on success, or NULL on error;
 */
m_dialog* new_m_dialog(str call_id)
{
	m_dialog *d;
	
	if (!m_dialog_count_increment()) return 0;
	d = shm_malloc(sizeof(m_dialog));
	if (!d) {
		LOG(L_ERR,"ERR:"M_NAME":new_m_dialog(): Unable to alloc %d bytes\n",
			sizeof(m_dialog));
		goto error;
	}
	memset(d,0,sizeof(m_dialog));
	
	d->hash = get_m_dialog_hash(call_id);		
...
out_of_memory:
	...
	m_dialog_count_decrement();
	return 0;
}

/**
 * Creates and adds a m_dialog to the hash table.
 * \note Locks the hash slot if ok! Call d_unlock(m_dialog->hash) when you are finished)
 * \note make sure that is_m_dialog(call_id) returns 0 or there will be unreachable duplicates!
...
m_dialog* add_m_dialog(str call_id)
{
	...
	d = new_m_dialog(call_id);
	if (!d) return 0;		
	
	d_lock(d->hash);
		d->next = 0;
		d->prev = m_dialogs[d->hash].tail;
		if (d->prev) d->prev->next = d;
		m_dialogs[d->hash].tail = d;
		if (!m_dialogs[d->hash].head) m_dialogs[d->hash].head = d;
...
{
	m_dialog *d=0;
	unsigned int hash = get_m_dialog_hash(call_id);

	d_lock(hash);
		d = m_dialogs[hash].head;
		while(d){
 ...
 * @returns the m_dialog* or NULL if not found
 */
...
{
	if (!mgcf_dialogs_enable_release) return 0;
	switch (d->method){
		...
		case DLG_METHOD_SUBSCRIBE:
			LOG(L_ERR,"ERR:"M_NAME":terminate_m_dialog(): Not needed for SUBSCRIBE dialogs - silent drop on expiration.\n");
			return 0;
			break;
		default:
			LOG(L_ERR,"ERR:"M_NAME":terminate_m_dialog(): Not implemented yet for method[%d]!\n",d->method);
			return 0;
	}
}
...
void del_m_dialog(m_dialog *d)
{
	...
	else m_dialogs[d->hash].head = d->next;
	if (d->next) d->next->prev = d->prev;
	else m_dialogs[d->hash].tail = d->prev;
	free_m_dialog(d);
}
...
void print_m_dialogs(int log_level)
{
	...
	LOG(log_level,"INF:"M_NAME":----------  MGCF Dialog List end   --------------\n");	
}

/**
 * Returns the m_dialog_direction from the direction string.
 * @param direction - "orig" or "term"
 * @returns the m_dialog_direction if ok or #DLG_MOBILE_UNKNOWN if not found
 */
...
int M_is_in_dialog(struct sip_msg* msg, char* str1, char* str2)
{
	...
	//print_m_dialog(L_ERR);
	call_id = cscf_get_call_id(msg,0);
	if (!call_id.len)
		return CSCF_RETURN_FALSE;
			
	if (is_m_dialog(call_id,&dir)) {
...
/**
 * Return m_dialog_method for a method string.
 * @param method - the string containing the method
 * @returns the m_dialog_method corresponding if known or #DLG_METHOD_OTHER if not
 */
static enum m_dialog_method get_dialog_method(str method)
{
 ...
 */	
static str get_dialog_method_str(enum m_dialog_method method)
{
	switch(method){
		case DLG_METHOD_INVITE:
			return s_INVITE;
...
int M_422_session_expires(struct sip_msg* msg, char* str1, char* str2)
{
	...
	if (!hdr.s) {
		LOG(L_ERR, "ERR:"M_NAME":M_422_session_expires(): no memory for hdr\n");
		goto error;
	}
	...
	if (!cscf_add_header_rpl(msg, &hdr)) {
		LOG(L_ERR, "ERR:"M_NAME":M_422_session_expires(): Can't add header\n");
		goto error;
 	}
...
int M_check_session_expires(struct sip_msg* msg, char* str1, char* str2)
{
	...
	if (!supports_extension(msg, &str_ext_timer)) //does not suports timer extension
	{
		...
		if (!new_min_se.s) {
			LOG(L_ERR,"ERR:"M_NAME":M_check_session_expires: Error allocating %d bytes\n",new_min_se.len);
			goto error;
		}
		new_min_se.len = snprintf(new_min_se.s, new_min_se.len, "%.*s %d\r\n",str_min_se.len, str_min_se.s, mgcf_min_se);
		min_se_time = mgcf_min_se;
		cscf_add_header(msg, &new_min_se, HDR_OTHER_T);
		if (t_time < mgcf_min_se) {
...
int M_save_dialog(struct sip_msg* msg, char* str1, char* str2)
{
	...
	LOG(L_DBG,"DBG:"M_NAME":M_save_dialog(%s): Call-ID <%.*s>\n",str1,call_id.len,call_id.s);

	if (is_m_dialog(call_id,&dir)){
		LOG(L_ERR,"ERR:"M_NAME":M_save_dialog: dialog already exists!\n");	
		return CSCF_RETURN_TRUE;
	}
	
	d = add_m_dialog(call_id);
 ...
 */
void save_dialog_routes(struct sip_msg* msg, char* str1,m_dialog *d)
{
	int i;
	rr_t *rr,*ri;	
 ...
 */
int update_dialog_on_reply(struct sip_msg *msg, m_dialog *d)
{
	...
	if (!t_time) //i.e not session-expires header in response
	{
		if (!d->uac_supp_timer || !d->lr_session_expires)
		{
			...
			{
				d->expires = d_act_time()+mgcf_dialogs_expiration_time;	
			}
		}
...
int M_update_dialog(struct sip_msg* msg, char* str1, char* str2)
{
	...
	LOG(L_DBG,"DBG:"M_NAME":M_update_dialog(%s): Call-ID <%.*s>\n",str1,call_id.len,call_id.s);

	d = get_m_dialog(call_id,&dir);
	if (!d)
		d = get_m_dialog(call_id,0);
	if (!d){
		if (msg->first_line.type==SIP_REQUEST &&
			...
			LOG(L_CRIT,"ERR:"M_NAME":M_update_dialog: dialog does not exists!\n");
			return CSCF_RETURN_FALSE;
		}	
	}
	...
	if (msg->first_line.type==SIP_REQUEST){
		/* Request */
		LOG(L_DBG,"DBG:"M_NAME":M_update_dialog(%s): Method <%.*s> \n",str1,
		...
		else
		{
			...
			{
				LOG(L_INFO,"INF:"M_NAME": update_dialog(%.*s): d->expires+=mgcf_dialogs_expiration_time 4\n",call_id.len,call_id.s);
				d->expires = d_act_time()+mgcf_dialogs_expiration_time;
			}
			d->lr_session_expires = 0;		
		}
		...
		LOG(L_DBG,"DBG:"M_NAME":M_update_dialog(%s): <%d> \n",str1,response);
		cseq = cscf_get_cseq(msg,&h);
		...
		if (d->first_cseq==cseq && d->method_str.len == ((struct cseq_body *)h->parsed)->method.len &&
			...
			if (response>=200 && response<300){
				...
				if (response>300){
					...
					d_unlock(d->hash);				
					return M_drop_dialog(msg,str1,str2);
				}				
		}else{
...
int M_drop_dialog(struct sip_msg* msg, char* str1, char* str2)
{
	...
	LOG(L_DBG,"DBG:"M_NAME":M_drop_dialog(%s): Call-ID <%.*s>\n",
		str1,call_id.len,call_id.s);
	...
	if (!d)
		d = get_m_dialog(call_id,0);
	if (!d){
		LOG(L_INFO,"INFO:"M_NAME":M_drop_dialog: dialog does not exists!\n");	
		return CSCF_RETURN_FALSE;
	}
...
//	int i,cnt=0;;
//	
//	LOG(L_DBG,"DBG:"M_NAME":M_drop_all_dialogs: Called for <%d://%.*s:%d>\n",transport,host.len,host.s,port);
//
//	for(i=0;i<m_dialogs_hash_size;i++){
//		d_lock(i);
//			d = m_dialogs[i].head;
...
int M_follows_dialog_routes(struct sip_msg *msg,char *str1,char *str2)
{
	...
	LOG(L_DBG,"DBG:"M_NAME":M_follows_dialog_routes(%s): Call-ID <%.*s>\n",
		str1,call_id.len,call_id.s);
	...
	if (!d)
		d = get_m_dialog(call_id,0);
	if (!d){
		LOG(L_ERR,"ERR:"M_NAME":M_follows_dialog_routes: dialog does not exists!\n");	
		return CSCF_RETURN_FALSE;
	}
	/* todo - fix this to match exactly the first request */
	if (d->first_cseq == cscf_get_cseq(msg,0) &&
		d->method == get_dialog_method(msg->first_line.u.request.method)){
		LOG(L_INFO,"INF:"M_NAME":M_follows_dialog_routes: this looks like the initial request (retransmission?)!\n");
		goto ok;		
	}
	...
	r = (rr_t*) hdr->parsed;	
	for(i=0;i<d->routes_cnt;i++){
		LOG(L_DBG,"DBG:"M_NAME":M_follows_dialog_routes:  must <%.*s>\n",
		...
		LOG(L_DBG,"DBG:"M_NAME":M_follows_dialog_routes: src %.*s\n",
		...
		{
			LOG(L_DBG,"DBG:"M_NAME":M_follows_dialog_routes: src match\n");		
		} else {
			LOG(L_DBG,"DBG:"M_NAME":M_follows_dialog_routes: found <%.*s>\n",
				r->nameaddr.uri.len,r->nameaddr.uri.s);					
			goto nok;
		}
		r = r->next;
	...
	if (r) {
		LOG(L_DBG,"DBG:"M_NAME":M_follows_dialog_routes: still has some extra Routes\n");		
		goto nok;
	}
	else 
...
/**
 * Enforce the proper dialog Request-URI
 * 
 */
int M_enforce_dialog_requri(struct sip_msg *msg,char *str1,char*str2)
{
	...
	LOG(L_INFO,"DBG:"M_NAME":M_enforce_dialog_requri(%s): Call-ID <%.*s>\n",
		str1,call_id.len,call_id.s);
	...
	if (!d)
		d = get_m_dialog(call_id,0);
	if (!d){
		LOG(L_ERR,"ERR:"M_NAME":M_enforce_dialog_requri: dialog does not exists!\n");	
		return CSCF_RETURN_FALSE;
	}
	
	if (!d->dialog_c || !d->dialog_c->rem_target.len){
		d_unlock(d->hash);
		LOG(L_ERR,"ERR:"M_NAME":M_enforce_dialog_requri: dialog does not have a dialog_c structure or the remote uri is empty!\n");	
		return CSCF_RETURN_FALSE;		
	}
	
	new_uri = d->dialog_c->rem_target;
	if (new_uri.s[0]=='<'){
		new_uri.s++;
		new_uri.len--;
	}
	if (rewrite_uri(msg, &(new_uri)) < 0) {
		LOG(L_ERR,"ERR:"M_NAME":M_enforce_dialog_requri: Error rewriting uri with <%.*s>\n",
...
int M_enforce_dialog_routes(struct sip_msg *msg,char *str1,char*str2)
{
	...
	LOG(L_INFO,"DBG:"M_NAME":M_enforce_dialog_routes(%s): Call-ID <%.*s>\n",
		str1,call_id.len,call_id.s);
	...
	if (!d)
		d = get_m_dialog(call_id,0);
	if (!d){
		LOG(L_ERR,"ERR:"M_NAME":M_enforce_dialog_routes: dialog does not exists!\n");	
		return CSCF_RETURN_FALSE;
	}
	...
	start = 0;
	for(i=0;i<d->routes_cnt;i++)
		if ((d->routes[i].len >= mgcf_record_route_mo_uri.len &&
	...
	x.len = route_s.len + route_e.len + (d->routes_cnt-1-start)*route_1.len;
	for(i=start;i<d->routes_cnt;i++)
		x.len+=d->routes[i].len;
			
	x.s = pkg_malloc(x.len);
	if (!x.s){
		LOG(L_ERR, "ERR:"M_NAME":M_enforce_dialog_routes: Error allocating %d bytes\n",
			x.len);
		x.len=0;
		d_unlock(d->hash);
		return CSCF_RETURN_ERROR;
	...
	for(i=start;i<d->routes_cnt;i++){
		if (i!=start) STR_APPEND(x,route_1);
	...
	newuri.len = d->routes[start].len;
	memcpy(newuri.s,d->routes[start].s,newuri.len);
	...
	if (cscf_add_header_first(msg,&x,HDR_ROUTE_T)) {
		if (cscf_del_all_headers(msg,HDR_ROUTE_T))
			...
			LOG(L_ERR,"ERR:"M_NAME":M_enforce_dialog_routes: new Route headers added, but failed to drop old ones.\n");
			return CSCF_RETURN_ERROR;
		}
	}
	else {
...
int M_record_route(struct sip_msg *msg,char *str1,char *str2)
{
	...
	switch (dir){
		...
		default:
			...
			pcscf.len = mgcf_name_str.len - scheme.len;
			...
			if (!rr.s){
				LOG(L_ERR,"ERR:"M_NAME":M_record_route: error allocating %d bytes!\n",rr.len);	
...
void dialog_timer(unsigned int ticks, void* param)
{
	...
	if (!m_dialogs) m_dialogs = (m_dialog_hash_slot*)param;
