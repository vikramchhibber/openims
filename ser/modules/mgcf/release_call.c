...
extern str mgcf_record_route_mo_body;
extern str mgcf_record_route_mt_body;
...
void alter_dialog_route_set(dlg_t *,enum m_dialog_direction);
int send_request(str ,str ,dlg_t *,transaction_cb , enum m_dialog_direction);
 ...
 * @param d - the m_dialog to end
 * @param reason - the Reason: header to include in the messages
 * @returns 0 on error 1 on success
 */
int release_call_confirmed(m_dialog *d, int reason_code, str reason_text)
{
	enum m_dialog_direction odir;
	m_dialog *o;
	str r;
	str hdrs={0,0};	
	char buf[256];
	...
	o = get_m_dialog_dir_nolock(d->call_id,odir);
...
{		
	...
	if (!ps->param) return;
	dir = *((enum m_dialog_direction *) *(ps->param));
	shm_free(*ps->param);
	*ps->param = 0;		
	...
	d = get_m_dialog_dir(call_id,dir);
	if (!d)	{
 ...
 */
int release_call_previous(m_dialog *d,enum release_call_situation situation,int reason_code,str reason_text)
{
	struct cell* t;
	m_dialog *o;
	enum m_dialog_direction odir;
...
int release_call_early(m_dialog *d,int reason_code,str reason_text)
{
...
int release_call_early200(m_dialog *d,int reason_code,str reason_text)
{
...
/**
 * Given a call-id, locate if its terminating,originating or both
...
int release_call(str callid,int reason_code,str reason_text)
{
	...
	d = get_m_dialog_dir(callid,DLG_MOBILE_ORIGINATING);
	if (d) {				
		hash = d->hash;
		if (release_call_p(d,reason_code,reason_text)>0) res = 1;		
		goto done;		
	}
	d = get_m_dialog_dir(callid,DLG_MOBILE_TERMINATING);
	if (d) {				
		hash = d->hash;
		if (release_call_p(d,reason_code,reason_text)>0) res = 1;		
		goto done;
...
int M_release_call(struct sip_msg *msg,char *str1,char *str2)
{
	...
	if (is_m_dialog_dir(callid,dir)) {
		d=get_m_dialog_dir(callid,dir);
		if (msg->first_line.type == SIP_REQUEST){
			d_unlock(d->hash);
			release_call(callid,RELEASE_CALL_ON_SUSPEND,reason);
...
*/
int send_request(str method,str reqbuf,dlg_t *d,transaction_cb cb, enum m_dialog_direction dir)
{
	if((d!=NULL) && (method.s!=NULL))
	{
		enum m_dialog_direction *cbp=NULL;
		...
		if (method.len !=3 || memcmp(method.s,"ACK",3)!=0)
		{
			...
			cbp = shm_malloc(sizeof(enum m_dialog_direction));
			if (!cbp){
				LOG(L_ERR,"ERR:"M_NAME":send_request(): error allocating %d bytes\n",sizeof(enum m_dialog_direction));
				return 0;
			}
			*cbp=dir;
		}	
 ...
 * This is required because half of the RRset is useless.
 * @param d - the dialog to modify the Record-Routes
 * @param dir - the direction
 */
void alter_dialog_route_set(dlg_t *d,enum m_dialog_direction dir)
{
	//rr_t *r;
	rr_t *r_new_pkg=0,*r_new_shm=0;	
	...
	switch (dir) {
		case DLG_MOBILE_ORIGINATING:
			p = mgcf_record_route_mo_body;
			break;
		case DLG_MOBILE_TERMINATING:
			p = mgcf_record_route_mt_body;
			break;
		default:
			return;
	}
...
//					
//	}	
	if (parse_rr_body(p.s,p.len,&r_new_pkg)<0)	return;
	if (shm_duplicate_rr(&r_new_shm,r_new_pkg)<0) {
		free_rr(&r_new_pkg);
		return;
	}	
	r_new_shm->next = d->route_set;
	d->route_set = r_new_shm;
}		
