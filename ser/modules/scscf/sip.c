 ...
 * @param buff - buffer allocated by caller used in case manipulation of identity is needed.
 *               i.e. sip carrying tel url, the identity returned would be tel url instead of sip
 * @param buff_len - length of the buffer
 * @returns 1 on success
 */
int cscf_get_terminating_identity(struct sip_msg *msg,str *uri, char *buff,
                                  int buff_len)
{
	struct sip_msg *req;	
	int i;
	...
	if (uri->len > buff_len) {
    LOG(L_ERR,"ERR:"M_NAME":cscf_get_terminating_identity: insufficient buffer length\n");
    return 0;
	}
	strncpy(buff, uri->s, buff_len);
	buff[uri->len] = '\0';
	
	/* @luminate - Look for "user=phone" */
	if (!strstr(buff, "user=phone")) {
	  for(i=0;i<uri->len;i++)
	    if (uri->s[i]==';' || uri->s[i]=='?') {
	      uri->len = i;
	      break;
	  ...
	  /* @luminate - case where tel url is carried by sip url
      tel url in the following two formats:
      sip:+<number>@<domain>;user=phone
      sip:<number>;phone-context=<context>@<domain>;user=phone */
	  static const char tel_prefix[] = "tel:";
	  /* Replace "sip:" with "tel:" */
      strncpy(buff, tel_prefix, sizeof(tel_prefix) - 1);
	  uri->s = buff;
	  uri->len = strlen(buff);
	  int copy_start = 0;
      LOG(L_INFO,"tel url found %.*s\n", uri->len, uri->s);

	  for(i = 0; i < uri->len; i++) {
        // Keep reading till we escape ":"
	    if (uri->s[i] == ':') {
	      copy_start = 1;
	      continue;
	    }
