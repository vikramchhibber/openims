...
/* @luminate - Fix to set data after escaping special xml chars */
void append_xml_data(str* dst, str* src)
{
  int i = 0;
  ...
  for (i = 0; i < src->len; i++) {
    switch (src->s[i])  {
      ...
      default: {
        char buf[16];
        snprintf(buf, sizeof(buf), "%c", src->s[i]);
        str data = {buf, 1};
        STR_APPEND((*dst), data); 
      }
    } /* switch */
					...
					if (p2){
						...
						while(c){
							...
							for(cp=c->parameters;cp;cp=cp->next){
								...
								pad.len = strlen(pad.s);
								STR_APPEND(buf,pad);
                                /* @luminate - escape the special xml characters */
								/* STR_APPEND(buf,cp->value); */
								append_xml_data(&buf, &cp->value);
