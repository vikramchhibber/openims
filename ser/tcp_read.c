...
/*
 * @luminate - Call to initialize tcp_connection receive address structure
 * params: con - already allocated tcp_connection structure
 * return: none
 */
inline static void initialize_tcp_connection(struct tcp_connection* con)
{
  int len = 0;
  char* tmp = 0;
  ...
  con->rcv.bind_address = (struct socket_info*) pkg_malloc(sizeof(struct socket_info));
  if (!con->rcv.bind_address) {
    LOG(L_ERR, "ERROR: initialize_tcp_connection out of memory for bind address");
    return;
  }
  memset(con->rcv.bind_address, 0, sizeof(struct socket_info));

  /* port */
  con->rcv.bind_address->port_no = con->rcv.dst_port;
  tmp = int2str(con->rcv.dst_port, &len);
  con->rcv.bind_address->port_no_str.s = (char*)pkg_malloc(len);
  if (!con->rcv.bind_address->port_no_str.s) {
    LOG(L_ERR, "ERROR: initialize_tcp_connection out of memory for bind address port");
    goto error;
  }
  strncpy(con->rcv.bind_address->port_no_str.s, tmp, len);
  con->rcv.bind_address->port_no_str.len = len;

  /* ip */
  if (!(tmp = ip_addr2a(&con->rcv.dst_ip))) {
    LOG(L_ERR, "ERROR: initialize_tcp_connection ip_addr2a failed");
    goto error;
  }
  len = strlen(tmp);
  con->rcv.bind_address->address_str.s = (char*)pkg_malloc(len);
  if (!con->rcv.bind_address->address_str.s) {
    LOG(L_ERR, "ERROR: initialize_tcp_connection out of memory");
    goto error;
  }
  strncpy(con->rcv.bind_address->address_str.s, tmp, len);
  con->rcv.bind_address->address_str.len = len;
  con->rcv.bind_address->address = con->rcv.dst_ip;
  con->rcv.bind_address->flags = SI_IS_IP;
  con->rcv.bind_address->proto = PROTO_TCP;
  con->rcv.bind_address->su = con->rcv.src_su;
  return;

error:
  if (con->rcv.bind_address->port_no_str.s)
    pkg_free(con->rcv.bind_address->port_no_str.s);
  con->rcv.bind_address->port_no_str.s = 0;
  if (con->rcv.bind_address->address_str.s)
    pkg_free(con->rcv.bind_address->address_str.s);
  con->rcv.bind_address->address_str.s = 0;
  if (con->rcv.bind_address)
    pkg_free(con->rcv.bind_address);
  con->rcv.bind_address = 0;
}
...
again:
			...
			/* @luminate - Initialize the socket's receive bind address structure */
			initialize_tcp_connection(con);
			con->fd=s;
			if (s==-1) {
...
/* @luminate For a given host and port, find and return the tcp_connection 
 * params: host: local ip 
 *         port: local port 
 *         proto: protocol (ignored) 
 * return: socket_info structure if found else null */
struct socket_info* get_tcp_socket_info(str* host, unsigned short port,
                        unsigned short proto)
{
  struct tcp_connection* con = 0;
  for (con=tcp_conn_lst; con; con=con->c_next) {
    if (con->rcv.bind_address->port_no != port) {
      continue;
    }
    if (con->rcv.bind_address->address_str.len != host->len) {
      continue;
    }
    if (!memcmp(con->rcv.bind_address->address_str.s, host->s, host->len)) {
      return con->rcv.bind_address;
    }
  }
